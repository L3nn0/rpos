      ...
      if (action == '/web/snapshot.jpg') {
        try {
          if (this.ffmpeg_process != null) {
            ...
            var cmd = `ffmpeg -fflags nobuffer -probesize 256 -rtsp_transport udp -i rtsp://${this.config.Username}:${this.config.Password}@${this.config.RTSPAddress}:${this.config.RTSPPort}/${this.config.RTSPName} -vframes 1  -r 1 -s 640x360 -y /dev/shm/snapshot.jpg`;
